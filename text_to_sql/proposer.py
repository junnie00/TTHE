...
def run_picker(tk):
    ...
    prompt = (
        ...
        f"2. NO CONSENSUS. NEVER treat the answer that MOST candidates agree on as correct — here the majority is "
        f"OFTEN WRONG. Judge each candidate independently against the DB and the Hint, NOT against what the other "
        f"candidates produced. The fact that several candidates return the same number is NOT evidence it is right.\n"
