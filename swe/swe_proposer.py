...
def propose_batch(tk):
    ...
    prompt = (
        ...
        f"can: change what the agent is told, change its budget, run it more than once, inspect the repo "
        f"yourself before or after a rollout, build your own checks, loop until a condition you define holds.\n"
        f"Harness API — these primitives EXIST; WHETHER, WHEN and HOW to use them is entirely up to you: "
        f"self.instance (dict); self.problem (issue text); self.repo; "
        f"self.run_agent(system_template=None, instance_template=None, step_limit=80, wall_time=1500) -> patch; "
        f"self._trace; and — most importantly, and easy to miss — "
        f"self.exec(command, timeout=None) -> {{output, returncode}}, which runs ONE BASH COMMAND IN THE REAL "
        f"REPO CONTAINER. That is a general, label-free execution primitive: you can read files, grep the "
        f"codebase, run the repo's own test suite, apply a patch and see what happens, or construct and run "
        f"a reproduction yourself. It is how you MANUFACTURE evidence instead of only consuming the agent's. "
        f"It carries no answer key — what runs is whatever you compose. NEVER use it to read the gold tests; "
        f"audit_harness.py rejects a candidate that does.\n"
